...
class Interpolation(nn.Module):
    def __init__(self, n_resblocks, n_feats, 
        ...
        self.ResidualGroup1=ResidualGroup(
                RCAB,
                ...
                act=act, 
                norm=norm)
        self.ResidualGroup2=ResidualGroup(
                RCAB,
                n_resblocks=n_resblocks,
                n_feat=n_feats,
                kernel_size=3,
                ...
                act=act, 
                norm=norm)
        self.ResidualGroup3=ResidualGroup(
                RCAB,
                n_resblocks=n_resblocks,
                n_feat=n_feats,
                kernel_size=3,
    ...
    def forward(self, x0, x1):
        ...
        x1 = self.ResidualGroup1(x)
        x2 = self.ResidualGroup2(x1)
        x3 = self.ResidualGroup3(x2)
        x3 += x

        out = self.tailConv(x3)
        return out,x2,x1
...
class Interpolation_decoder(nn.Module):
    def __init__(self, n_resblocks, n_feats, 
        ...
        # define modules: head, body, tail
        self.headConv1 = conv3x3(n_feats * 2, n_feats)
        self.headConv2 = conv3x3(n_feats * 2, n_feats)
        self.headConv = conv3x3(n_feats * 2, n_feats)
        self.ResidualGroup4=ResidualGroup(
                RCAB,
                n_resblocks=n_resblocks,
                n_feat=n_feats,
                kernel_size=3,
                ...
                act=act, 
                norm=norm)
        self.ResidualGroup5=ResidualGroup(
                RCAB,
                n_resblocks=n_resblocks,
                n_feat=n_feats,
                kernel_size=3,
                ...
                act=act, 
                norm=norm)
        self.ResidualGroup6=ResidualGroup(
                RCAB,
                n_resblocks=n_resblocks,
                n_feat=n_feats,
                kernel_size=3,
    ...
    def forward(self, x3,x2,x1):
        x = self.headConv(x3)
        out1 = self.ResidualGroup4(x)
        x2=out1-x2
        out1 =  torch.cat([x2, out1], dim=1)
        out1 = self.headConv1(out1)
        out2 =  self.ResidualGroup5(out1)
        x1 = out2-x1
        out2 = torch.cat([x1, out2], dim=1)
        out2 = self.headConv2(out2)
        out = self.ResidualGroup6(out2)
