...
import numpy as np            
from basicmodel.basic_x4.han  import HAN as model1           
from basicmodel.basic_x4.cain  import CAIN as model2   
...
class parallel_model(nn.Module):
    ...
    def forward(self,x,m_items,train=False):#x:volume,(pici,chang,kuang,gao)
            lr_volume=x.float().cuda(0)#1,5,9,13
            b,c,k,g = lr_volume.shape
            interpolate_out=[]
            for j in range(0,lr_volume.shape[3]-1):
                slice0=lr_volume[:,:,:,j]
                ...
                for h in range(0,middle_slice_interpolate.shape[1]):
                    interpolate_out.append(middle_slice_interpolate[:,h,:,:])
            middle_slice_sr_left=self.model1(lr_volume.unsqueeze(1).permute(0,2,1,3,4).view(-1,1,k,g))[:,:,:,:-3]
            ...
            middle_slice_sr_right=self.model1(lr_volume.unsqueeze(1).permute(0,3,1,2,4).view(-1,1,c,g))[:,:,:,:-3]
            ...
            res_volume=[]
            j=0
            for i in range(0,min(middle_slice_sr_left.shape[3],middle_slice_sr_right.shape[3])):
