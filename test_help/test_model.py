...
import numpy as np  
from basicmodel.basic_x2.han  import HAN as model1           
from basicmodel.basic_x2.cain  import CAIN as model2     
...
class parallel_model(nn.Module):
    ...
    def forward(self,x,m_items,train=False):#x:volume,(pici,chang,kuang,gao)
            lr_volume=x.float().cuda(0)#1,3,5,7,9,11,13
            ...
            middle_slice_sr_left=self.model1(lr_volume.unsqueeze(1).permute(0,2,1,3,4).view(-1,1,k,g))[:,:,:,:-1]
            ...
            middle_slice_sr_right=self.model1(lr_volume.unsqueeze(1).permute(0,3,1,2,4).view(-1,1,c,g))[:,:,:,:-1]
            ...
            res_volume=[]
            j=0
            #print(len(interpolate_out))
            #print(middle_slice_sr_left.shape)
