...
from testx4_model import parallel_model as model        
from data import *
...
def test(args):
    model.cuda(0)
    m_items=torch.load(r"/home/liangwang/Desktop/cvpr_ct_sr/checkpoint/memory_items/x4/"+"8keys.pt").cuda(0)
    pretrained_dict = torch.load(r"/home/liangwang/Desktop/cvpr_ct_sr/checkpoint/logs/x4/"+"8_model.pk")
    ...
    for  i, input_volume in enumerate(dataloader): 
        ...
        lr_volume = input_volume[:,:,:,::2].float().cuda(0)
        with torch.no_grad():
            interpolation_out_2 = model(lr_volume.detach(),m_items.detach())
        interpolation_out = interpolation_out_2.cuda(0).squeeze()
        out_volume = interpolation_out.clip(0,1).squeeze()
